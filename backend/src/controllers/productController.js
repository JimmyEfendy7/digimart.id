...
const getAllProducts = async (req, res) => {
  try {
    ...
    if (simpleProducts.length > 0) {
      const firstProduct = simpleProducts[0];
      console.log('🔍 [DEBUG] First product sample:', {
        id: firstProduct.id,
        name: firstProduct.name,
        is_active: firstProduct.is_active,
        is_verified: firstProduct.is_verified,
        store_id: firstProduct.store_id,
        poster_url: firstProduct.poster_url
      });
      
      // Check if the image file actually exists on filesystem
      if (firstProduct.poster_url) {
        try {
          const imagePath = path.join(__dirname, '../../public', firstProduct.poster_url);
          console.log('🔍 [DEBUG] Checking image path:', imagePath);
          const imageExists = await fs.access(imagePath).then(() => true).catch(() => false);
          console.log('🔍 [DEBUG] Image file exists:', imageExists);
          ...
          console.log('🔍 [DEBUG] Error checking image file:', error.message);
        }
      }
    }
    ...
    try {
      ...
      const storesDir = path.join(__dirname, '../../public/stores');
      
      console.log('🔍 [DEBUG] Public directory path:', publicDir);
      
      const publicExists = await fs.access(publicDir).then(() => true).catch(() => false);
      const productsExists = await fs.access(productsDir).then(() => true).catch(() => false);
      const reviewsExists = await fs.access(reviewsDir).then(() => true).catch(() => false);
      const storesExists = await fs.access(storesDir).then(() => true).catch(() => false);
      
      console.log('🔍 [DEBUG] Directory existence:', {
        public: publicExists,
        products: productsExists,
        reviews: reviewsExists,
        stores: storesExists
      });
      
      if (productsExists) {
        const productFiles = await fs.readdir(productsDir);
        console.log('🔍 [DEBUG] Product files count:', productFiles.length);
        if (productFiles.length > 0) {
          console.log('🔍 [DEBUG] First few product files:', productFiles.slice(0, 5));
        }
      }
    } catch (error) {
      console.log('🔍 [DEBUG] Error checking directories:', error.message);
    }
