...
class WhatsAppService {
  ...
  async initialize() {
    ...
    try {
      ...
      // Prioritaskan Chrome dengan PUPPETEER_EXECUTABLE_PATH dari ENV bila disediakan
      const envBrowserPath = process.env.PUPPETEER_EXECUTABLE_PATH;
      if (envBrowserPath) {
        console.log('📱 Menggunakan PUPPETEER_EXECUTABLE_PATH dari ENV:', envBrowserPath);
        puppeteerOptions.executablePath = envBrowserPath;
      }

      // Jika path dari ENV gagal nanti, kita akan fallback ke pencarian path di sistem
      const chromePaths = [
        // ENV override — tetap coba juga dalam daftar
      ...
      // Coba tes browser terlebih dahulu dengan puppeteer langsung (gunakan env jika ada)
      try {
        ...
        console.error('❌ Gagal meluncurkan browser langsung (env path mungkin tidak valid):', browserError);
        // Bersihkan executablePath untuk memicu fallback pencarian
        delete puppeteerOptions.executablePath;

        // Lakukan pencarian path Chrome/Edge di sistem dan pilih yang tersedia
        console.log('📱 Melanjutkan dengan pencarian path Chrome/Edge di sistem...');
